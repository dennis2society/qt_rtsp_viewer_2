...
{
    QImage tmp = img;
    if (tmp.format() != QImage::Format_RGB888 &&
        tmp.format() != QImage::Format_Grayscale8)
        tmp = tmp.convertToFormat(QImage::Format_RGB888);

    cv::Mat mat(tmp.height(), tmp.width(),
...
QImage OpenCVProcessor::matToQImage(const cv::Mat &mat, QImage::Format fmt)
{
    if (mat.channels() == 3) {
        ...
        return QImage(m_rgbMat.data, m_rgbMat.cols, m_rgbMat.rows,
                      static_cast<int>(m_rgbMat.step), fmt).copy();
    }
...
// ─────────────────────────────────────────────────────────────────────────────
QImage OpenCVProcessor::applyBrightnessContrast(const QImage &src,
...
// ─────────────────────────────────────────────────────────────────────────────
QImage OpenCVProcessor::applyMotionDetectionOverlay(const QImage &drawTarget,
                                                     const QImage &cleanCurrent,
                                                     const QImage &cleanPrevious,
                                                     int sensitivity)
{
    if (cleanPrevious.isNull()) return drawTarget;

    cv::Mat cur = qImageToMat(cleanCurrent);
    cv::Mat prev;
    {
        QImage tmp = cleanPrevious.convertToFormat(QImage::Format_RGB888);
        cv::Mat raw(tmp.height(), tmp.width(), CV_8UC3,
                    const_cast<uchar *>(tmp.constBits()),
                    static_cast<size_t>(tmp.bytesPerLine()));
...
// ─────────────────────────────────────────────────────────────────────────────
QImage OpenCVProcessor::applyMotionVectorsOverlay(const QImage &drawTarget,
                                                    const QImage &cleanCurrent,
                                                    const QImage &cleanPrevious)
{
    if (cleanPrevious.isNull()) return drawTarget;
    ...
    cv::Mat flow;
    cv::calcOpticalFlowFarneback(smallPrev, smallCur, flow,
                                  0.5, 2, 9, 2, 5, 1.1, 0);
...
// Face detection
// ─────────────────────────────────────────────────────────────────────────────
QImage OpenCVProcessor::applyFaceDetection(const QImage &drawTarget,
                                            const QImage &cleanCurrent)
{
...
// ─────────────────────────────────────────────────────────────────────────────
double OpenCVProcessor::computeMotionLevel(const QImage &cleanCurrent,
                                            const QImage &cleanPrevious,
                                            int sensitivity)
{
    if (cleanPrevious.isNull()) return 0.0;
    ...
    for (int row = 0; row < kGridRows; ++row) {
        for (int col = 0; col < kGridCols; ++col) {
            ...
            if (!m_medianHistory.empty()) {
                std::vector<double> sorted(m_medianHistory.begin(),
                                           m_medianHistory.end());
...
// ─────────────────────────────────────────────────────────────────────────────
QImage OpenCVProcessor::applyGridMotionOverlay(const QImage &drawTarget,
                                                const QImage &cleanCurrent,
                                                const QImage &cleanPrevious,
                                                int sensitivity)
{
...
// ─────────────────────────────────────────────────────────────────────────────
QImage OpenCVProcessor::applyMotionGraphOverlay(const QImage &drawTarget,
                                                 double motionLevel)
{
    ...
    QString pct = QStringLiteral("Motion: %1 %").arg(
        static_cast<int>(motionLevel * 100));
    ...
    for (int row = 0; row < kGridRows; ++row) {
        ...
        p.drawText(lx + 14, gY + 13 + row * 14,
                   QStringLiteral("Row %1").arg(row + 1));
    }
