...
// ─────────────────────────────────────────────────────────────────────────────
RecordDialog::RecordDialog(const QString &defaultDir,
...
void RecordDialog::setupUI(const QString &defaultDir, const QString &cameraName)
{
    ...
    connect(m_presetCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, [this](int idx) {
                if (idx < 0 || idx >= s_presets.size()) return;
...
void RecordDialog::onBrowse()
{
    ...
    QString ext = (idx >= 0 && idx < s_presets.size())
                      ? s_presets[idx].ext : QStringLiteral("mp4");
    QString filter = QStringLiteral("Video (*.%1);;All files (*)").arg(ext);
    QString path = QFileDialog::getSaveFileName(this,
                       QStringLiteral("Save recording"), m_pathEdit->text(), filter);
    if (!path.isEmpty())
        m_pathEdit->setText(path);
}

QString RecordDialog::filePath() const { return m_pathEdit->text(); }

QString RecordDialog::codec() const
{
    int idx = m_presetCombo->currentIndex();
    return (idx >= 0 && idx < s_presets.size()) ? s_presets[idx].codec
                                                 : QStringLiteral("libx264");
}

double RecordDialog::fps() const { return m_fpsSpin->value(); }
