...
void RecordingWorker::stopRecording()
{
    ...
    while (drained < remaining && drained < kMaxDrainFrames && !m_interrupt.load(std::memory_order_relaxed)) {
        QImage img;
        {
            QMutexLocker lk(&m_queueMutex);
            if (m_queue.isEmpty())
...
{
    int ret = avformat_alloc_output_context2(&m_fmtCtx, nullptr, nullptr, m_recPath.toUtf8().constData());
    if (ret < 0 || !m_fmtCtx)
        return false;
    ...
    if (ret < 0) {
        ...
        if (isNvenc || isVaapi || isQsv) {
            ...
            // Retry with software
            m_recCodec = isHevc ? QStringLiteral("libx265") : QStringLiteral("libx264");
    ...
    if (m_fmtCtx->oformat && m_fmtCtx->oformat->name) {
        ...
        if (fmt.find("mp4") != std::string::npos || fmt.find("mov") != std::string::npos) {
    ...
    m_swsCtx = sws_getContext(w, h, AV_PIX_FMT_RGB24, w, h, AV_PIX_FMT_YUV420P, SWS_FAST_BILINEAR, nullptr, nullptr, nullptr);
...
void RecordingWorker::writeFrame(const QImage &img)
{
    ...
    sws_scale(m_swsCtx, srcSlice, srcStride, 0, rgb.height(), m_avFrame->data, m_avFrame->linesize);
